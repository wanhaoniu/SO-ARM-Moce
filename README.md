...
## 1. 项目简介
**SOARM-Moce** 是我们在 **SOARM101** 基础上进行的二次开发版本：在保持 **相同 5 自由度（DOF）架构** 与 **Python + ROS 控制方式** 不变的前提下，通过对关键关节引入**金属减速模组强化**，显著提升了承载能力与结构刚度，同时扩大了工作空间覆盖范围。:contentReference[oaicite:1]{index=1}
...
## 3. 升级亮点（相对 SOARM101）
- **负载能力跃升**：通过关键关节金属减速模组强化，实现负载能力显著提升（实际实验获得）。:contentReference[oaicite:2]{index=2}  
- **工作空间扩大**：基于公开 URDF 仿真评估，工作空间面积提升接近 30%。:contentReference[oaicite:3]{index=3}  
- **结构刚度与稳定性增强**：强化结构带来更强抗扭与抗变形能力，提升系统稳定性。:contentReference[oaicite:4]{index=4}  
- **精度与控制习惯保持一致**：重复定位精度保持 1 mm，控制方式保持 Python + ROS，不改变上手成本。:contentReference[oaicite:5]{index=5}  
- **生态更完整**：兼容 **LeRobot** 上游生态，并扩展 **Moce 自有生态支持**。:contentReference[oaicite:6]{index=6}  

---

## 4. 核心指标对比（SOARM101 vs SOARM-Moce）
> 以下数据整理自项目对比材料：负载为实验结果，工作空间相关为 URDF 仿真结果。:contentReference[oaicite:7]{index=7}
